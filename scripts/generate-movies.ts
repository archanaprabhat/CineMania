...
async function main() {
  ...
  const allGenres = [
    ...movieGenres,
    ...tvGenres.filter((g) => !movieGenres.find((mg) => mg.id === g.id)),
  ];
  ...
  let allShows = Array.from(
    new Map(
      [...trendingShows, ...topRatedShows, ...onTheAirShows].map((s) => [
        s.id,
        s,
      ]),
    ).values(),
  ...
  await fs.writeFile(
    path.join(dataDir, "genres.json"),
    JSON.stringify(allGenres, null, 2),
  );
  await fs.writeFile(
  ...
  console.log(
    `Saved ${allGenres.length} genres, ${allMovies.length} movies, ${allShows.length} shows, and ${actors.length} actors to ${dataDir}`,
  );
}

...
export default async function ShowDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="min-h-screen bg-background pb-12">
      ...
      >
        <GlassBadge icon={Calendar}>{new Date(show.first_air_date).getFullYear()}</GlassBadge>
        {show.episode_run_time && show.episode_run_time.length > 0 && (
          <GlassBadge icon={Clock}>{formatRuntime(show.episode_run_time)}</GlassBadge>
        )}
        <GlassBadge icon={Star} variant="yellow">{show.vote_average.toFixed(1)}</GlassBadge>
        {(show.number_of_seasons || 0) > 0 && (
            <GlassBadge icon={Layers}>{show.number_of_seasons} Seasons</GlassBadge>
        )}
